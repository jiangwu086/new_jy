    ...
    <el-card shadow="never" v-loading="loading">
      ...
      <div class="review-grid" v-else>
        <div class="review-card" v-for="item in list" :key="item.id">
          ...
          <div class="card-body">
            <div class="info-row">
              <span class="label">姓名</span><span>{{ item.userName || '-' }}</span>
            </div>
            <div class="info-row">
              <span class="label">手机号</span><span>{{ item.userPhone }}</span>
            </div>
            <div class="info-row">
              <span class="label">打卡时间</span><span>{{ fmt(item.checkinTime) }}</span>
            </div>
            <div class="info-row">
              <span class="label">获得积分</span>
              <span style="color:#10b981;font-weight:600">+{{ item.pointsEarned }}</span>
            </div>
          ...
          <div class="card-actions" v-if="item.reviewStatus === 0">
            <el-button size="small" :loading="item._approving"
...
import { getPendingReviews, reviewCheckin } from '@/api/index.js'
...
async function doReview(item, action, remark = '') {
  ...
  } finally { item._approving = false; item._rejecting = false }
}

function openReject(item) { rejectTarget.value = item; rejectRemark.value = ''; rejectVisible.value = true }

async function confirmReject() {
  if (!rejectRemark.value.trim()) { ElMessage.warning('请填写驳回原因'); return }
  rejecting.value = true
  try { await doReview(rejectTarget.value, 'reject', rejectRemark.value); rejectVisible.value = false }
  finally { rejecting.value = false }
}

const statusLabel = s => s === 0 ? '待审核' : s === 1 ? '已通过' : '已驳回'
const statusClass = s => s === 0 ? 'pending' : s === 1 ? 'approved' : 'rejected'
...
.page-header h3 { font-size:18px; color:#0f1f2e; }
.review-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(210px,1fr)); gap:16px; }
.review-card { border:1px solid #ebeef5; border-radius:10px; overflow:hidden; background:#fff; transition:box-shadow .2s; }
.review-card:hover { box-shadow:0 4px 16px rgba(0,0,0,.1); }
.photo-wrap { position:relative; height:170px; background:#f5f7fa; }
.photo-img { width:100%; height:100%; }
.photo-placeholder { display:flex; align-items:center; justify-content:center; height:100%; color:#ccc; font-size:14px; }
.status-tag { position:absolute; top:8px; right:8px; padding:2px 10px; border-radius:20px; font-size:12px; font-weight:600; }
.status-tag.pending  { background:#fff7ed; color:#f59e0b; }
.status-tag.approved { background:#f0fdf4; color:#10b981; }
